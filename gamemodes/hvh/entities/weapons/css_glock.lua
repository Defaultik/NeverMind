...
SWEP.Purpose			= "Now this is a good pistol, right?"
...
SWEP.Primary.Sound			= Sound("weapons/glock/glock-1.wav")
...
function SWEP:PrimaryAttack()		
	...
	local bullet = {}	-- Set up the shot
		...
		self:EmitSound( Sound("weapons/glock/glock18-1.wav") )
...
local ground = self.Owner:GetGroundEntity()
		if self.Owner:KeyDown( IN_FORWARD ) and !self.Owner:KeyDown( IN_DUCK ) and (ground:IsWorld() or ground:IsWorld()) then
			...
			self.Primary.Cone = 1.45
			else
			if !ground:IsWorld() or !ground:IsWorld() then
			self.Primary.Cone = 4.8
			else
			self.Primary.Cone = 2.9
							end
							end
							end
							end
...
function SWEP:CSShootBullet( dmg, recoil, numbul, cone )
	...
	self:SetNextPrimaryFire( self.Primary.Fire )
