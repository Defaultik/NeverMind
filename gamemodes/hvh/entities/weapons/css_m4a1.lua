-- I used the weapons's worldmodel from the spawnmenu's abysses to make the MekoAR2 :)
...
SWEP.Purpose			= "My favourite weapon! ...After the M249 !"
SWEP.PrintName			= "M4A1"
SWEP.Slot				= 2
SWEP.SlotPos			= 3
...
function SWEP:PrimaryAttack()		
	...
	local bullet = {}	-- Set up the shot
		...
		if (self.Zoom == 1) then
			self:EmitSound( Sound("weapons/m4a1/m4a1-1.wav") )
			self.Weapon:SendWeaponAnim(ACT_VM_PRIMARYATTACK_SILENCED)
			self.Owner:SetAnimation( PLAYER_ATTACK1 )
		else
			self:EmitSound( Sound("weapons/m4a1/m4a1_unsil-1.wav") )
